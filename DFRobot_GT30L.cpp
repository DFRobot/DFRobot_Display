...
uint32_t DFRobot_GT30L::getGTAddr(uint8_t* pGB2312, eCHARACTER_HEIGHT type)
{
  ...
  if((pGB2312[0] & 0x80) != 0x80) {
    if( pGB2312[0] >= ' ' )
    temp = pGB2312[0] - ' ';
    if( type == eCHARACTER_HEIGHT_8 )        temp = temp*8  + ASC0808D2HZ_ADDR;      //7*8 ascii code
    else if( type == eCHARACTER_HEIGHT_12 )  temp = temp*12 + ASC0812M2ZF_ADDR;      //6*12 ascii code
    else if( type == eCHARACTER_HEIGHT_16 )  temp = temp*16 + GBEX0816ZF_ADDR;     //8*16 ascii code
  } else {
